...
fn test_emergency_pause_admin_override_event_reconciles_with_persisted_state() {
    ...
    // The re-entrancy lock is always cleared by a successful call.
    let lock_after: Option<bool> = env.as_contract(&contract_id, || {
        env.storage().instance().get(&DataKey::EmergencyPauseLock)
    });
    assert_eq!(lock_after, Some(false));
    let paused_after: Option<bool> = env.as_contract(&contract_id, || {
        env.storage().instance().get(&DataKey::EmergencyPaused)
    });
