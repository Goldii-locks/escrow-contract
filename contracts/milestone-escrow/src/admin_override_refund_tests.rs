//! Authorization and source-state guards for `admin_override_refund`
//! (issue #329).
//!
//! An unauthorized caller and an illegal (paused) source state must each
...
fn snapshot_refund_ledger(
    ...
    let paused = env.as_contract(&client.address, || {
        env.storage()
            .instance()
            .get::<_, bool>(&crate::DataKey::Paused)
            .unwrap_or(false)
    });
    RefundLedgerSnapshot {
        paused,
...
fn unauthorized_caller_returns_unauthorized_and_does_not_mutate_storage() {
    ...
    let (client_addr, _, _, _admin_addr, token_id, _, client) =
...
#[test]
fn illegal_paused_source_state_returns_paused_and_does_not_mutate_storage() {
    let env = Env::default();
    env.mock_all_auths();
...
fn client_freelancer_and_arbiter_are_unauthorized_without_storage_mutation() {
    ...
    let (client_addr, freelancer_addr, arbiter_addr, _admin_addr, token_id, _, client) =
        setup_funded_escrow(&env, vec![&env, 1_000_i128]);

    let before = snapshot_refund_ledger(&env, &client, &token_id, &client_addr);
    for caller in [client_addr.clone(), freelancer_addr, arbiter_addr] {
        let result = client.try_admin_override_refund(&caller, &0u32);
        assert_eq!(result, Err(Ok(Error::Unauthorized)));
        let after = snapshot_refund_ledger(&env, &client, &token_id, &client_addr);
        assert_ledger_unchanged(&before, &after);
    }
...
#[test]
fn admin_override_refund_refunds_client() {
    let env = Env::default();
    env.mock_all_auths();
