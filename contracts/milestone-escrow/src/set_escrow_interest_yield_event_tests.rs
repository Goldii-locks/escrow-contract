...
use crate::{Error, EscrowInterestYieldSetEvent};
use soroban_sdk::{
    symbol_short, testutils::Address as _, vec, Address, Env, FromVal, IntoVal, Val,
};
...
fn set_yield_unauthorized_no_event() {
    ...
    // rejected call must have emitted none (the setup call's event is not
    // counted here).
    assert_eq!(event_count(&env), 0, "no event on unauthorized call");
}
